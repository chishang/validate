    ...
    S.extend(Verify, Base, /** @lends verify.prototype*/{
        ...
        _verify: function (field, rules) {
            ...
            for (var i = 0, len = rules.length; i < len; i++) {
                var rule = rules[i];
                var skiped = self.get('skiped')[field];

                if (disabled[field]) {
                    return fieldResult;
                }
                if (skiped && S.indexOf(i, skiped) !== -1) {
                    continue;
                }
         ...
         * @param index {Array}
         */
        skip: function (field, index) {
            var self = this;
            if (S.isArray(index)) {
                S.each(index, function (i) {
                    self._skip(field, i);
                });
            }
            else {
                self._skip(field, index);
            }
        },
        _skip: function (field, index) {
            var self = this;
            var skiped = self.get('skiped');
            if (!skiped[field]) {
                skiped[field] = [];

            }
            ;
            if (!S.inArray(index, skiped[field])) {
                skiped[field].push(index);
            }
